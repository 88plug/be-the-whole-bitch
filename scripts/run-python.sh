...
find_python() {
  ...
  for c in "${EIGHTYEIGHT_PYTHON:-}" "${PLUGIN_PYTHON:-}" "${BTWB_PYTHON:-}"; do
    _try "$c" && return 0
  done
...
PY="$(find_python)" || {
  echo "be-the-whole-bitch: no Python >=${MIN_MAJOR}.${MIN_MINOR} found." >&2
  echo "  Set EIGHTYEIGHT_PYTHON=/path/to/python3 (or BTWB_PYTHON), or install Python 3." >&2
